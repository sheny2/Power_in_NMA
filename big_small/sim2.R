...
indirect_result_big <- expand_grid(pi_a, OR_ab, OR_ac, tau) %>% 
  mutate(k_ab = k_ab, k_ac = k_ac, k_bc = k_bc) %>% 
...
# Apply the function in parallel using foreach
direct_result_big <- expand_grid(pi_a, OR_ab, OR_ac, tau) %>% 
  mutate(k_ab = k_ab, k_ac = k_ac, k_bc = k_bc) %>% 
  mutate(OR_bc = round(exp(log(OR_ac) - log(OR_ab)), 2)) %>% 
  mutate(pi_b = pi_a * OR_ab / (1 - pi_a + pi_a * OR_ab )) %>% 
  mutate(pi_c = pi_a * OR_ac / (1 - pi_a + pi_a * OR_ac )) %>% 
...
save(direct_result_big, file = "direct_result_big.RData")
...
# Apply the function in parallel using foreach
direct_result_small <- expand_grid(pi_a, OR_ab, OR_ac, tau) %>% 
  mutate(k_ab = k_ab, k_ac = k_ac, k_bc = k_bc) %>% 
  mutate(OR_bc = round(exp(log(OR_ac) - log(OR_ab)), 2)) %>% 
  mutate(pi_b = pi_a * OR_ab / (1 - pi_a + pi_a * OR_ab )) %>% 
  mutate(pi_c = pi_a * OR_ac / (1 - pi_a + pi_a * OR_ac )) %>% 
...
save(direct_result_small, file = "direct_result_small.RData")
...
### In the setting of overall evidence
