...
# Define a function to be applied in parallel
power_mutate <- function(df) {
  df %>%  mutate(power = bigsmall.sim.bias(N_sim, k_ab, k_ac, k_bc, pi_a, OR_ab, OR_ac,tau,total = 3000))
}
...
# Define a function to be applied in parallel
power_mutate <- function(df) {
  df %>%  mutate(power = bigsmall.sim.bias(N_sim, k_ab, k_ac, k_bc, pi_a, OR_ab, OR_ac,tau,total = 30000))
}
...
save(overall_result_big_small, file = "result_extreme_small30000/overall_result_big_small_new.RData")
