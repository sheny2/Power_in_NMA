...
power_mutate <- function(df) {
  df %>%  mutate(power = power.sim.NMA2(N_sim, k_ab, k_ac, k_bc = 0, pi_a, OR_ab, OR_ac, tau))
}
...
save(df_indirect_new, file = "df_indirect_BNMA.RData")
...
# Define a function to be applied in parallel
power_mutate <- function(df) {
  df %>%  mutate(power = power.sim.NMA2(N_sim, k_ab = 0, k_ac = 0, k_bc, pi_a, OR_ab, OR_ac, tau))
}
...
save(df_direct_new, file = "df_direct_BNMA.RData")
...
# Define a function to be applied in parallel
power_mutate <- function(df) {
  df %>%  mutate(power = power.sim.NMA2(N_sim, k_ab, k_ac, k_bc, pi_a, OR_ab, OR_ac, tau))
}
...
save(df_BNMA_new, file = "df_overall_BNMA.RData")
