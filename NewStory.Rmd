...
simple_2 = df_BNMA_bias0.5_sub %>% filter(k_ab == 6) %>% 
 ...
 geom_line(data = df_BNMA_bias0.5_sub %>% filter(k_bc == 2, k_ab < 12), size = 1, aes(y = avg_bias_abs, x = k_ab, color = "Complete NMA (Add Indirect)")) + 
  geom_line(data = df_direct_summ0.5_sub[which(df_direct_summ0.5_sub$k_bc %in% c(1,2,3,4,6)),], size = 1, aes(y = avg_bias_abs, x = k_bc, color = "Direct Evidence")) + 
  geom_line(data = df_indirect_summ0.5_sub[which(df_indirect_summ0.5_sub$k_ab %in% c(1,2,3,4,6)),], size = 1, aes(y = avg_bias_abs, x = k_ab, color = "Indirect Evidence (Two arm)")) + 
