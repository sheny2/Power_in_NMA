...
df_indirect_bias <- expand_grid(pi_a, OR_ab, OR_ac, tau, k_ab) %>% 
  mutate(OR_bc = round(exp(log(OR_ac) - log(OR_ab)), 2)) %>%
  mutate(pi_b = pi_a * OR_ab / (1 - pi_a + pi_a * OR_ab )) %>% 
...
save(df_indirect_bias, file = "df_indirect_BNMA_bias2.RData")
...
# Define a function to be applied in parallel
power_mutate <- function(df) {
  df %>%  mutate(power = power.sim.NMA4(N_sim, k_ab = 0, k_ac = 0, k_bc, pi_a, OR_ab, OR_ac, tau))
}

# Apply the function in parallel using foreach
df_direct_bias <- expand_grid( k_bc, pi_a, OR_ab, OR_ac, tau) %>% 
  mutate(OR_bc = round(exp(log(OR_ac) - log(OR_ab)), 2)) %>% 
  mutate(pi_b = pi_a * OR_ab / (1 - pi_a + pi_a * OR_ab )) %>% 
  mutate(pi_c = pi_a * OR_ac / (1 - pi_a + pi_a * OR_ac )) %>% 
...
save(df_direct_bias, file = "df_direct_BNMA_bias2.RData")
